  ...
  : rclcpp::Node("grasp_detection_server", options)
{
  ...
  if (!rviz_topic.empty()) {
    ...
    {
      std::cerr << e.what() << '\n';
      RCLCPP_ERROR(this->get_logger(), "Failed to initialize GraspPlotter: %s", e.what());
      use_rviz_ = false;
    }
  ...
  auto cb = [this](const std::shared_ptr<DetectConstrainedGrasps::Request> req,
    ...
    RCLCPP_INFO(get_logger(), "===========DetectConstrainedGrasps service called.===========");          
    this->handleRequest(req, res);
  };
  service_ = this->create_service<DetectConstrainedGrasps>("detect_constrained_grasps", cb);
...
void GraspDetectionServer::handleRequest(const Request req, Response res)
{
  ...
  if (cloud_sources.cloud.fields.size() == 6 && cloud_sources.cloud.fields[3].name == "normal_x" && cloud_sources.cloud.fields[4].name == "normal_y" && cloud_sources.cloud.fields[5].name == "normal_z")
  {
    ...
    RCLCPP_INFO(get_logger(), "Created gpd::util::Cloud ...");
  ...
  // Keep requested samples as XYZ coordinates, not indices. GPD's fork
  // filters and voxelizes cloud_processed_ during preprocessing, but does not
  // correctly remap externally supplied sample indices through those changes.
  // Coordinates are filtered and subsampled independently, so they continue to
  // refer to the food points selected by the caller.
  const auto &original_cloud = cloud_camera_->getCloudOriginal();
  Eigen::Matrix3Xd samples(3, req->cloud_indexed.indices.size());
  for (size_t i = 0; i < req->cloud_indexed.indices.size(); ++i) {
    const auto index = req->cloud_indexed.indices[i].data;
    if (index < 0 || static_cast<size_t>(index) >= original_cloud->size()) {
      RCLCPP_ERROR(get_logger(),
                   "Sample index %ld is outside cloud of size %zu.", index,
                   original_cloud->size());
      return;
    }
    samples.col(i) = original_cloud->points[index].getVector3fMap().cast<double>();
  }
  ...
  RCLCPP_INFO(get_logger(), "Preprocessed point cloud ...");
  ...
  if (req->params_policy == gpd_ros2_msgs::srv::DetectConstrainedGrasps::Request::USE_CFG_FILE) {
    RCLCPP_INFO(get_logger(), "Using cfg file ...");
    ...
    RCLCPP_INFO(get_logger(), "Using request parameters ...");
  ...
  if (!grasps.empty()) {
    ...
    for (size_t i = 0; i < msg.grasps.size(); ++i) {
      const auto &grasp = msg.grasps[i];
      RCLCPP_INFO(get_logger(),
                  "Grasp %zu: score %f, approach=(%.4f, %.4f, %.4f), "
                  "width=%.4f",
                  i, grasp.score.data, grasp.approach.x, grasp.approach.y,
                  grasp.approach.z, grasp.width.data);
    }
    RCLCPP_INFO(get_logger(), "=====================Finished service request.==================");
    return;
  }
