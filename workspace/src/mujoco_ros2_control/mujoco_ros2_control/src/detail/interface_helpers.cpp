    ...
    if (auto it = sensor_hardware_info.find(sensor.name); it != sensor_hardware_info.end())
    {
      for (const auto& state_if : it->second.state_interfaces)
      {
        ...
        else if (state_if.name.find("orientation_covariance") == 0)
        {
          ...
          {
            interfaces.emplace_back(sensor.name, state_if.name, &sensor.orientation_covariance[idx]);
          }
        }
        else if (state_if.name.find("angular_velocity_covariance") == 0)
        {
          ...
          {
            interfaces.emplace_back(sensor.name, state_if.name, &sensor.angular_velocity_covariance[idx]);
          }
        }
        else if (state_if.name.find("linear_acceleration_covariance") == 0)
        {
          ...
          {
            interfaces.emplace_back(sensor.name, state_if.name, &sensor.linear_acceleration_covariance[idx]);
          }
        }
      }
    }
