...
private:
  ...
  /**
   * @brief Process paused rendering and requested manual steps.
   */
  void run_paused_simulation(int force_buffer_size);

  void initialize_force_buffers();
  bool handle_ui_reset(mjtNum previous_sim_time);
  void capture_viewer_forces(int force_buffer_size);
  void copy_control_inputs();
  void compose_external_forces(int force_buffer_size);
  void restore_external_forces_for_viewer(int force_buffer_size);
