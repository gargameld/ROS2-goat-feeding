 ...
 *
 * Converts lidar extension sensor data from the physics simulation into the
 * appropriate ROS message. Single-row sensors publish LaserScan messages;
 * multi-row sensors publish PointCloud2 messages.
 */
...
private:
  /**
   * @brief Build and register the configuration for one `mujoco.plugin.lidar` sensor.
   */
