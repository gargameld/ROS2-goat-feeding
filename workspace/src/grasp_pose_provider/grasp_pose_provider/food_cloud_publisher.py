...
:func:`grasp_pose_provider.food_detector.detect_food` already segmented out of
the combined cloud, and ``sensors_3d.yaml`` subscribes to that topic alone.
...
# The topic ``sensors_3d.yaml`` reads the octomap from. Keep the two in sync.
...
def food_cloud_message(cloud_msg, food_indices, stamp=None):
    ...
    :func:`~grasp_pose_provider.food_detector.detect_food` produces. Points
    that are non-finite are dropped, so the result is a dense XYZ cloud in
    ``cloud_msg``'s frame. ``stamp`` defaults to ``cloud_msg``'s own stamp.
    """
...
class FoodCloudPublisher:
    ...
    def __init__(self, node, topic=DEFAULT_FOOD_CLOUD_TOPIC):
        self._node = node
        self._topic = topic
